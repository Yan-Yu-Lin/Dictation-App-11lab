...
- **Floating status indicator**: Small on-screen dot shows recording (red) / finalizing (orange) state
- **Live preview**: The dot expands into a capsule showing partial transcripts as you speak (wraps up to 4 lines), already converted to Traditional Chinese
...
Disable Right Shift push-to-talk if needed:

```bash
uv run dictation.py --disable-right-shift-ptt
```
...
- **Right Shift (hold)**: Push-to-talk start while held, stop on release
